...
public class KubernetesReplicationControllersProducerTest extends CamelTestSupport {
    ...
    public void createAndDeleteService() throws Exception {
        ...
        Exchange ex = template.request("direct:createReplicationController", new Processor() {
			...
			public void process(Exchange exchange) throws Exception {
				...
				EditablePodTemplateSpec t = builder.withNewMetadata().withName("nginx-template").addToLabels("server", "nginx").endMetadata()
		          .withNewSpec()
		          .addNewContainer().withName("wildfly").withImage("jboss/wildfly")
    ...
    protected RouteBuilder createRouteBuilder() throws Exception {
        return new RouteBuilder() {            
            ...
            public void configure() throws Exception {
                from("direct:list")
                        .toF("kubernetes://%s?username=%s&password=%s&category=replicationControllers&operation=listReplicationControllers",host,username,password);
                from("direct:listByLabels")
                        .toF("kubernetes://%s?username=%s&password=%s&category=replicationControllers&operation=listReplicationControllersByLabels",host,username,password);
                from("direct:getReplicationController")
                        .toF("kubernetes://%s?username=%s&password=%s&category=replicationControllers&operation=getReplicationController",host,username,password);
                from("direct:createReplicationController")
                        .toF("kubernetes://%s?username=%s&password=%s&category=replicationControllers&operation=createReplicationController",host,username,password);
                from("direct:deleteReplicationController")
                        .toF("kubernetes://%s?username=%s&password=%s&category=replicationControllers&operation=deleteReplicationController",host,username,password);
            } 
        };
    }
}
