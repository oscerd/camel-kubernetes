...
public class KubernetesPersistentVolumesClaimsProducerTest extends CamelTestSupport {
    ...
    protected RouteBuilder createRouteBuilder() throws Exception {
        return new RouteBuilder() {            
            ...
            public void configure() throws Exception {
                from("direct:list")
                        .toF("kubernetes://%s?username=%s&password=%s&category=persistentVolumesClaims&operation=listPersistentVolumesClaims",host,username,password);
                from("direct:listByLabels")
                        .toF("kubernetes://%s?username=%s&password=%s&category=persistentVolumesClaims&operation=listPersistentVolumesClaimsByLabels",host,username,password);
                from("direct:create")
                        .toF("kubernetes://%s?username=%s&password=%s&category=persistentVolumesClaims&operation=createPersistentVolumeClaim",host,username,password);
                from("direct:delete")
                        .toF("kubernetes://%s?username=%s&password=%s&category=persistentVolumesClaims&operation=deletePersistentVolumeClaim",host,username,password);
            } 
        };
    }
}
